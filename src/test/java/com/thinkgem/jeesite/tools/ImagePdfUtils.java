...
public class ImagePdfUtils {
	public static void main(String[] args) throws Exception {
		...
		imgMerageToPdf(new File("F:\\GitHub\\jeesite\\src\\main\\webapp\\userfiles\\images\\tm\\report\\201608040008").listFiles(),
				new File("F:\\GitHub\\jeesite\\src\\main\\webapp\\userfiles\\images\\tm\\report\\201608040008.pdf"));
	}
	...
	public static boolean imgMerageToPdf(File[] list, File file) throws Exception {
		...
		for (File f : list){
			if(f.getName().indexOf("large") > 0) {
				mif.put(Integer.parseInt(f.getName().replaceAll("-large.jpg$", "")), f);
			}
		}
		...
		for (Entry<Integer, File> eif : mif.entrySet()) {
			// 4.1:读取到内存中
			baos = new ByteArrayOutputStream(2048 * 5);
